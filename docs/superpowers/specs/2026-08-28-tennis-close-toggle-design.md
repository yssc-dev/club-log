...
- (1차 a5f7182) 버튼 라벨 `마감 해제` → `← 경기로 돌아가기`. `{!finalized && …}` 조건 그대로.
- (2차, 유저 지시 "풋살처럼 하단에 경기로·구글시트전송·아카이브") 상단 뒤로가기 버튼 제거, 하단 바를 풋살 최종집계와 같은 **한 줄** `[경기로] [기록확정 (구글시트 전송)] [아카이브]` 로 재배치. "경기로"는 `onBack`(화면만 내림), 전송 완료 후 숨김. 아카이브는 전송 완료+관리자일 때만 활성. 렌더 테스트 `tennisSummaryView.render.test.jsx` 4건.
