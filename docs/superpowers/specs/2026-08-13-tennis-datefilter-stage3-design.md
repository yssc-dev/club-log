...
| 기본값 | **올해(2026)** |
| 레거시 연도(2024/2025) | **순위(승/패 집계)만 + 안내**, 로우 기반 섹션 숨김. 레거시 모드에선 **선수 select 숨김**(의미 없음) |
| 연도별전적 카드 | **전체 커리어(2024~+통산) 항상 표시** — 필터 무관(2026-08-13 적대적 리뷰 후 "필터 연도만" 결정 번복: 필터로 좁히면 1행 퇴화·요약과 중복이라, 기간별 비교라는 카드 본래 목적 유지) |
