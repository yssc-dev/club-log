...
5. **참석자 단계(컵 분기, `isCup`일 때만):** 선수 칩을 시즌 명단이 아니라 **대회 팀별로 묶어** 그린다(`teams[i]`의 이름, 팀명 헤더, 기본 전원 선택, 토글은 기존 `TOGGLE_ATTENDEE`). 팀마다 "당일 추가" 입력: 이름을 `attendees`와 그 팀 `teams[i]`에 함께 넣는다(이미 어느 팀에든 있으면 무시). 숨김: `시트 연동`·`활동선수 전체`. `초기화`·정렬 토글은 컵에서 숨김(칩을 개별로 해제한다). 경기 설정: `팀 수` 세그먼트 비활성(대회 팀 수), `팀 편성 방식`은 "대회 팀" 한 칸(비활성)으로 대체, `경기 모드`는 대진표만(자유대진·밀어내기 비활성), `구장 수` 1/2(기존 규칙: 3팀 이하 1구장 고정), **`회전 수` 1~3은 구장 수와 무관하게 표시**(정규는 1구장일 때만). 안내 문구: `풀리그 × {rotations}회전 · 참석 {M}팀 · {라운드 수}라운드`. 하단 CTA 라벨 `대회 팀 확인 ({M}팀)`, 활성 조건 = 참석자 1명 이상인 팀이 2개 이상.
