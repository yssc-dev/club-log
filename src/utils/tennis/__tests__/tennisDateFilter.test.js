...
describe('buildLegacyStandings', () => {
  ...
  it('순위는 승률 우선(승률↓→승수↓): 승수가 많아도 승률이 낮으면 뒤 — 단·복식 공통', () => {
    const rows = [
      { player: 'X', season: 2025, format: '복식', wins: 3, losses: 0 },   // 100%지만 3승
      { player: 'Y', season: 2025, format: '복식', wins: 8, losses: 6 },   // 57%지만 8승
    ];
    expect(buildLegacyStandings({ legacyRows: rows, year: '2025', format: '복식' }).map(x => x.name)).toEqual(['X', 'Y']);
    const singles = rows.map(r => ({ ...r, format: '단식' }));
    expect(buildLegacyStandings({ legacyRows: singles, year: '2025', format: '단식' }).map(x => x.name)).toEqual(['X', 'Y']);
  });
